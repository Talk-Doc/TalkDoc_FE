...
import { Calendar, Check, AlertCircle, Keyboard, Hand } from 'lucide-react'
...
// 환자 상황과 완전히 맞는 보기가 없을 수 있습니다. 백엔드 QuestionResponse 문서에 "CARD_SELECT는
// card_options 카드 + '직접 작성'/'수어로 답변' 탈출구를 같이 보여주라"고 명시돼 있어서, 텍스트뿐
// 아니라 수어로도 답할 수 있는 탈출구를 함께 보여줍니다.
    ...
    <>
      ...
      {cardOptions.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-2 text-center">
          ...
          <p className="text-xs text-slate-400">아래 버튼으로 직접 답변해주세요.</p>
        </div>
      ) : (
      ...
      <div className="mt-auto flex flex-col gap-2">
        ...
        <p className="text-center text-xs text-slate-400">맞는 보기가 없다면 다른 방법으로 답변할 수 있어요.</p>
        <div className="flex gap-2">
          <button
            onClick={onSwitchToSign}
            className="flex-1 flex items-center justify-center gap-1.5 py-3 rounded-xl border border-slate-200 text-slate-500 font-medium"
          >
            <Hand size={16} />
            수어로 답변
          </button>
