...
// clamp의 최소값은 min(640px, 90vh)로 둬서, 창 세로가 유난히 짧을 때도(예: 가로만
// 넓히고 세로는 그대로인 창) 카드가 뷰포트보다 커져 스크롤이 생기는 일이 없게 했습니다.
  ...
  return (
    <div className="min-h-screen bg-white sm:bg-slate-100 flex items-center justify-center sm:p-4">
      <div
        ...
        className="a11y-scope w-full h-full min-h-screen sm:min-h-0 sm:w-auto sm:h-[clamp(min(640px,90vh),88vh,900px)] sm:aspect-[9/19] bg-white sm:rounded-3xl sm:shadow-lg flex flex-col p-5 sm:p-6 lg:p-8 overflow-y-auto"
      >
        {children}
      </div>
    </div>
